...
authorized_devices = [('FE:36:EA:1E:62:AF', 'sensor_1'), ('E4:F2:81:30:8D:52', 'sensor_3')]
...
def publish_upstream(client, mac, payload):
    ...
    for sensor in authorized_devices:
        if mac == sensor[0]:
            ...
            logging.info(f"publishing message for {sensor[1]} ({mac}).")
            client.publish(f"$iothub/{sensor[1]}/messages/events", str(payload), qos=1)
    if(is_authorized_device == False):
        logging.warning(f"ignoring message from an unknown sensor (MAC address: {mac}).")
...
def main():
    ...
    RuuviTagSensor.get_datas(callback, [sensor[0] for sensor in authorized_devices])
